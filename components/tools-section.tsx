...
function ToolCard({ tool }: { tool: AITool }) {
  ...
  return (
    <Link href={`/tools/${tool.id}`} className="block">
      <Card className={`group relative flex flex-col bg-card border-border/50 transition-all duration-300 hover:border-border hover:shadow-xl ${glowClass} hover:-translate-y-0.5 cursor-pointer`}>
        {tool.popular && (
        ...
        <CardContent className="flex-1 pb-3">
          <div className="flex flex-wrap gap-1.5">
            {tool.tags.map((tag) => (
              <Badge key={tag} variant="secondary" className="text-xs px-2 py-0.5 bg-secondary/50 text-muted-foreground hover:bg-secondary transition-colors">
                {tag}
              </Badge>
            ))}
          </div>
    ...
    <>
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                <span>{cat.emoji}</span>
                <span>{cat.label}</span>
                <span className={`rounded-full px-1.5 py-0.5 text-xs font-mono leading-none ${isSelected ? "bg-background/20 text-current" : "bg-muted text-muted-foreground"}`}>
                  {count}
                </span>
              </Button>
            );
      ...
      <div className="mb-6 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {filteredTools.length === 0 ? "검색 결과가 없습니다" : (
            <>
              <span className="font-semibold text-foreground">{filteredTools.length}</span>개의 도구
              {searchQuery && <span className="ml-1">— &quot;<span className="text-primary">{searchQuery}</span>&quot; 검색 결과</span>}
            </>
          )}
        </p>
        {(searchQuery || selectedCategory !== "all") && (
          <Button variant="ghost" size="sm" className="text-xs text-muted-foreground hover:text-foreground h-auto py-1" onClick={() => { setSearchQuery(""); setSelectedCategory("all"); }}>
            필터 초기화
          </Button>
        )}
      </div>
